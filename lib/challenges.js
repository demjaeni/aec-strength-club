...
export const CORE_CATEGORIES = ['physical', 'faith', 'mindset', 'character'];

// Consecutive completed days counting back from today. A not-yet-ticked
// "today" doesn't break the streak — it's only a gap once its day has passed.
export function currentStreak(completedDays, dayNum) {
  let day = Math.min(dayNum, 60);
  if (!completedDays.has(day)) day -= 1;
  let streak = 0;
...
// Longest run of consecutive day-numbers ever completed, computed fresh from
// completions each time (safe since completion rows are never deleted).
export function longestStreak(completedDays) {
  const days = [...completedDays].sort((a, b) => a - b);
  let longest = 0;
  let run = 0;
  let prev = null;
  for (const d of days) {
...
export function categoryDays(category) {
  return CHALLENGES.filter((ch) => ch.category === category).map((ch) => ch.day);
}

export function categoryStats(completedDays) {
  return CORE_CATEGORIES.map((category) => {
    const days = categoryDays(category);
    const done = days.filter((d) => completedDays.has(d)).length;
    return {
      category,
      done,
      total: days.length,
      percent: days.length ? Math.round((done / days.length) * 100) : 0,
    };
  });
}
...
export const CHALLENGES = [
  ...
  c(13, '❤️', 'faith', 'Spend a quiet moment seeking sincere forgiveness from Allah for shortcomings made today'),
  ...
  c(22, '❤️', 'faith', 'Attend Fajr Club'),
  c(23, '🤝', 'character', "Give something in charity, even if it's £1."),
  c(24, '💪', 'physical', 'Hold a wall sit for 2 minutes.'),
  ...
  { day: 46, icon: '💪', category: 'physical', intro: 'Complete:', items: ['10 squats', '5 push-ups', '20-second plank'], after: 'Repeat for 5 rounds.' },
